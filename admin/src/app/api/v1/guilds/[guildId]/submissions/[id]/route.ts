...
export async function GET(_req: Request, ctx: { params: Promise<{ guildId: string; id: string }> }) {
  ...
  if (!sub || sub.guildId !== guildId) return NextResponse.json({ error: 'not_found' }, { status: 404 });
